...
\subsubsection{Participantes e Perfis}

A Tabela~\ref{tab:participantes} apresenta os participantes e suas atribuições no estudo.
...
\subsubsection{Roteiro de Tarefas}

O walkthrough foi estruturado em três sessões sequenciais, simulando o fluxo real de uso do sistema:

\textbf{Sessão 1 -- Administrador (Parte 1):}
\begin{itemize}
  \item Login como administrador
  \item Importar planilha de planejamento do DCC
  \item Verificar professores notificados automaticamente
  \item Visualizar projetos criados (status: pendente de assinatura)
\end{itemize}

\textbf{Sessão 2 -- Professor:}
\begin{itemize}
  \item Primeiro acesso: recuperação de senha via ``Esqueci minha senha''
  \item Login e completar onboarding (SIAPE, CPF, assinatura digital)
  \item Visualizar projeto criado pela importação
  \item Criar/editar template padrão para a disciplina
  \item Editar projeto do semestre (bolsas, voluntários, atividades)
  \item Pré-visualizar PDF, assinar digitalmente e submeter
\end{itemize}

\textbf{Sessão 3 -- Administrador (Parte 2):}
\begin{itemize}
  \item Visualizar projeto submetido pelo professor
  \item Analisar detalhes e aprovar o projeto
  \item Gerar planilha consolidada para Instituto/PROGRAD
\end{itemize}

Após cada walkthrough, foi conduzida uma entrevista semiestruturada de aproximadamente 15 minutos, explorando a experiência geral do participante, comparação com o processo manual atual, estimativa de economia de tempo, barreiras percebidas para adoção em semestre real e sugestões de melhoria. As perguntas foram organizadas por categoria de análise (experiência geral, funcionalidades específicas por perfil, comparação, economia de tempo, adoção e sugestões), conforme apresentado na Tabela~\ref{tab:roteiro-entrevista}. Esta abordagem seguiu as recomendações de Lazar et al.~\cite{lazar2017research} para condução de estudos qualitativos em IHC, permitindo explorar em profundidade as percepções e necessidades dos usuários enquanto mantém foco nas dimensões relevantes para validação do sistema.
...
\begin{table*}[htb]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tblr}{
      width=\textwidth,
      colspec={|Q[c,1.8cm]|Q[c,1.5cm]|X[l]|},
      hlines,
      vlines
    }
      \textbf{Categoria} & \textbf{Perfil} & \textbf{Pergunta} \\
...
A análise temática das verbalizações e entrevistas, seguindo o método de Braun e Clarke~\cite{braun2006using}, identificou três categorias principais: usabilidade, utilidade e adoção.

\begin{figure*}[!htb]
  \centering
  \begin{tikzpicture}[
    node distance=0.8cm,
    category/.style={rounded corners, draw=gray!70, fill=blue!10, minimum width=3.5cm, minimum height=1.2cm, align=center, font=\small\bfseries},
    subcategory/.style={rounded corners, draw=gray!50, fill=white, minimum width=3.2cm, text width=3cm, align=left, font=\scriptsize},
    title/.style={rounded corners, draw=gray!70, fill=yellow!20, minimum width=10cm, minimum height=0.9cm, align=center, font=\small\bfseries},
    >={Stealth[length=2mm]}
  ]
    % Título
    ...
    % Categorias
    \node[category, below left=1.2cm and 2.5cm of title] (usab) {Usabilidade\\{\scriptsize (+4, -1)}};
    \node[category, below=1.2cm of title] (util) {Utilidade\\{\scriptsize (+3)}};
    \node[category, below right=1.2cm and 2.5cm of title] (adoc) {Adoção\\{\scriptsize (+2, S:3)}};

    % Subcategorias Usabilidade
    \node[subcategory, below=0.5cm of usab] (usab1) {
      \textbullet~Sidebar organizado\\
      \textbullet~Onboarding claro\\
      \textbullet~PDF preview útil\\
      \textbullet~Assinatura intuitiva\\
      {\color{orange}\textbullet~Reset senha$^*$}
    };

    % Subcategorias Utilidade
    \node[subcategory, below=0.5cm of util] (util1) {
      \textbullet~Templates reutilizáveis\\
      \textbullet~Importação automática\\
      \textbullet~Planilha PROGRAD OK
    };

    % Subcategorias Adoção
    \node[subcategory, below=0.5cm of adoc] (adoc1) {
      \textbullet~Projeto continuará\\
      \textbullet~Uso em semestre real\\
      {\color{blue}\textbullet~Notificar professor$^\dagger$}\\
      {\color{blue}\textbullet~Reposicionar planilha$^\dagger$}
    };

    % Conectores
    \draw[->] (title.south) -- ++(0,-0.4) -| (usab.north);
    \draw[->] (title.south) -- (util.north);
    \draw[->] (title.south) -- ++(0,-0.4) -| (adoc.north);

    % Legenda
    \node[below=3.5cm of title, font=\scriptsize, text width=12cm, align=center] {
      (+) pontos positivos \quad (-) dificuldades \quad (S) sugestões\\[0.2em]
...
A Figura~\ref{fig:thematic-map} apresenta o mapa temático resultante da análise, sintetizando os principais achados por categoria.
...
\begin{table}[htb]
  ...
  \begin{tblr}{
    width=\linewidth,
    colspec={|X[l]|Q[c,1.8cm]|X[l]|},
    hlines,
    vlines
  }
    \textbf{Objetivo Específico} & \textbf{Status} & \textbf{Evidência} \\
