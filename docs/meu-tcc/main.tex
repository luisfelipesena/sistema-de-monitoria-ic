...
\subsubsection{Roteiro de Entrevista}

A Tabela~\ref{tab:roteiro-entrevista} apresenta as perguntas utilizadas nas entrevistas semiestruturadas, organizadas por categoria de análise.
...
Os resultados da avaliação qualitativa serão apresentados após a realização das sessões com os participantes. A análise seguirá a estrutura de análise temática, identificando:

\textbf{Perfil Administrador:}
\begin{itemize}
  \item Pontos positivos identificados: \textit{TODO}
  \item Dificuldades encontradas: \textit{TODO}
  \item Sugestões de melhoria: \textit{TODO}
\end{itemize}
...
\begin{itemize}
  \item Padrões comuns entre os perfis: \textit{TODO}
  \item Divergências de percepção: \textit{TODO}
\end{itemize}

\textbf{Validação dos Objetivos Específicos:}

A Tabela~\ref{tab:validacao-objetivos} apresentará a validação dos objetivos específicos com base nas evidências coletadas.
...
\begin{table}[htb]
  ...
  \begin{tblr}{
    width=\linewidth,
    colspec={|X[l]|Q[c,2cm]|X[l]|},
    hlines,
    vlines
  }
    \textbf{Objetivo Específico} & \textbf{Status} & \textbf{Evidência} \\
    Digitalizar ciclo completo de projetos & \textit{TODO} & \textit{TODO} \\
    Automatizar processo seletivo & \textit{TODO} & \textit{TODO} \\
    Sistematizar alocação de bolsas & \textit{TODO} & \textit{TODO} \\
    Eliminar trabalhos manuais repetitivos & \textit{TODO} & \textit{TODO} \\
    Fornecer base analítica para decisões & \textit{TODO} & \textit{TODO} \\
  \end{tblr}
\end{table}
