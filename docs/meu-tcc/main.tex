...
\begin{table*}[htb]
  ...
  \begin{adjustbox}{max width=\textwidth}
    \begin{tabular}{|l|l|p{8cm}|}
      ...
      \hline
      \multicolumn{3}{|l|}{\footnotesize \textsuperscript{*}RF13 implementado: relatórios finais e certificados completos (código pronto, validação com usuários pendente).} \\
      \hline
...
A comunicação entre camadas segue padrões bem estabelecidos que asseguram consistência e rastreabilidade das operações. O cliente React consome endpoints tRPC que, internamente, invocam serviços de negócio responsáveis por orquestrar chamadas a repositórios e a APIs externas (envio de e-mails via Nodemailer, armazenamento de arquivos no MinIO). Os dados trafegam sempre validados por esquemas Zod compartilhados entre cliente e servidor, eliminando divergências de tipo e prevenindo inconsistências. Erros originados em qualquer camada (validação de entrada, regras de negócio, falhas de persistência) são propagados de forma estruturada até o cliente, que apresenta mensagens adequadas ao contexto de uso. A arquitetura ainda contempla pontos de extensão planejados para integrações futuras, como autenticação via CAS (Central Authentication Service) da universidade, sincronização automática de dados acadêmicos com sistemas legados e exportação de métricas para ferramentas de business intelligence externas, garantindo que o sistema possa crescer em funcionalidade sem necessidade de reestruturação fundamental.

Do ponto de vista de escalabilidade, a arquitetura permite crescimento horizontal tanto na camada de aplicação quanto na de dados. O uso de containers Docker facilita a replicação de instâncias da aplicação atrás de um balanceador de carga, enquanto o PostgreSQL suporta réplicas de leitura para distribuir consultas pesadas. O MinIO pode ser configurado em modo distribuído para alta disponibilidade de arquivos. Essa flexibilidade arquitetural garante que o sistema possa atender a demandas crescentes sem redesenho significativo. A separação entre lógica de negócio e persistência também facilita a substituição de componentes específicos: o banco de dados poderia ser migrado para soluções gerenciadas em nuvem (como Amazon RDS ou Google Cloud SQL) sem alterações no código da aplicação, e o armazenamento de objetos poderia transicionar para Amazon S3 ou Google Cloud Storage mantendo compatibilidade via protocolo S3.

Do ponto de vista operacional, o sistema foi projetado para facilitar monitoramento e diagnóstico em ambiente de produção. Os logs estruturados em formato JSON permitem integração com ferramentas de observabilidade como Elasticsearch, Grafana ou Datadog, possibilitando alertas automáticos em caso de degradação de performance ou erros recorrentes. As métricas de uso (requisições por segundo, tempo de resposta, taxa de erros) podem ser exportadas via endpoints dedicados, alimentando dashboards operacionais que auxiliam a equipe de infraestrutura na identificação proativa de gargalos. A estratégia de cache implementada no cliente (via React Query) reduz significativamente a carga sobre o servidor, enquanto índices otimizados no banco de dados garantem tempos de resposta consistentes mesmo sob picos de demanda característicos de períodos de inscrição.

A arquitetura contempla ainda mecanismos de resiliência que minimizam o impacto de falhas parciais. Transações de banco de dados são utilizadas para garantir atomicidade em operações críticas, como a submissão de projetos e a alocação de bolsas, evitando estados inconsistentes mesmo em caso de interrupções inesperadas. O sistema implementa padrões de \textit{retry} com backoff exponencial para chamadas a serviços externos (envio de e-mails, armazenamento de arquivos), de modo que falhas transitórias não resultem em perda de dados ou operações incompletas. Filas de processamento assíncrono permitem que tarefas demoradas (geração de relatórios, envio em lote de notificações) sejam executadas em segundo plano sem bloquear a interface do usuário, melhorando a experiência percebida e a responsividade geral do sistema.

O tratamento de erros segue uma estratégia hierárquica que propaga exceções de forma controlada através das camadas arquiteturais. Erros de validação detectados na camada de apresentação são exibidos imediatamente ao usuário com mensagens contextualizadas, enquanto falhas de regras de negócio na camada de serviços são transformadas em códigos de erro padronizados que o cliente interpreta para apresentar feedback apropriado. Exceções não previstas são capturadas por handlers globais que registram informações diagnósticas detalhadas (stack trace, contexto da requisição, estado do usuário) antes de retornar respostas genéricas que não expõem detalhes internos da implementação, preservando a segurança do sistema.

A gestão de configurações adota o princípio de separação entre código e ambiente, utilizando variáveis de ambiente para parâmetros que variam entre ambientes de desenvolvimento, homologação e produção. Credenciais sensíveis (chaves de API, senhas de banco de dados, tokens de autenticação) são injetadas em tempo de execução através de gerenciadores de segredos (\textit{secrets managers}) ou arquivos de ambiente protegidos, nunca sendo armazenadas no repositório de código. Essa abordagem facilita a implantação em diferentes infraestruturas e permite que a mesma imagem Docker seja promovida entre ambientes sem necessidade de recompilação, seguindo práticas recomendadas de DevOps e integração contínua.

O versionamento da API segue convenções semânticas que permitem evolução gradual sem quebrar clientes existentes. Mudanças compatíveis (adição de campos opcionais, novos endpoints) são incorporadas transparentemente, enquanto alterações incompatíveis (remoção de campos, mudança de tipos) são sinalizadas através de mecanismos de depreciação que concedem período de transição aos consumidores. A documentação da API é gerada automaticamente a partir dos esquemas Zod e das definições tRPC, garantindo que a especificação técnica permaneça sincronizada com a implementação real e reduzindo o risco de divergências entre documentação e comportamento efetivo do sistema.
...
O modelo de dados foi projetado para capturar todas as entidades e relacionamentos do domínio de monitoria, seguindo princípios de normalização e integridade referencial. O esquema relacional adota a terceira forma normal (3NF), eliminando redundâncias e garantindo que cada atributo dependa exclusivamente da chave primária de sua tabela. Chaves estrangeiras com restrições de integridade referencial asseguram consistência entre entidades relacionadas, impedindo operações que resultariam em registros órfãos ou referências inválidas. O modelo contempla ainda índices compostos estrategicamente posicionados nas colunas mais frequentemente consultadas, otimizando operações de busca e agregação que sustentam os dashboards e relatórios do sistema.

A modelagem foi orientada pelos fluxos de negócio identificados durante o levantamento de requisitos, priorizando a rastreabilidade de operações e a auditabilidade de mudanças de estado. Cada entidade principal mantém campos de metadados temporais (\texttt{created\_at}, \texttt{updated\_at}) que permitem reconstruir o histórico de modificações. Relacionamentos muitos-para-muitos foram decompostos em tabelas associativas que capturam atributos específicos da associação (por exemplo, a relação entre projeto e disciplina inclui informações sobre o papel de cada disciplina no projeto). A Figura \ref{fig:data-model} apresenta as principais entidades e seus relacionamentos:
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{images/Assinatura de Projeto.png}
  \caption{Assinatura de projeto pelo professor com prévia do PDF.}
  \label{fig:professor-assinatura}
\end{figure}

Para estudantes, a tela de inscrição (Figura \ref{fig:student-inscricao}) oferece filtros por departamento e tipo de vaga, busca por título/professor e exibe avisos sobre o período ativo; quando o período está fechado, os componentes permanecem acessíveis para consulta e planejamento. Após criar um projeto, o professor pode editá-lo através da interface de edição (Figura \ref{fig:editar-projeto}), que permite ajustar informações do plano de monitoria, atividades previstas e configurações de vagas antes da submissão.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{images/Editar Projeto De Monitoria.png}
  \caption{Interface de edição de projeto de monitoria pelo professor.}
  \label{fig:editar-projeto}
\end{figure}
...
Completam as interfaces administrativas a gestão de usuários (Figura \ref{fig:admin-users}), onde a administração pesquisa, filtra e gerencia contas mantendo coerência com regras institucionais; a importação de planejamento (Figura \ref{fig:admin-import}), que processa planilhas do planejamento e emite relatório de inconsistências; e a tela de departamentos (Figura \ref{fig:admin-departamentos}), que permite manter a estrutura acadêmica essencial para vínculos de disciplinas e projetos. A interface de análise de projetos (Figura \ref{fig:admin-analise}) permite ao administrador revisar projetos submetidos pelos professores, visualizando detalhes do plano de monitoria, informações da disciplina e dados do professor responsável antes de aprovar ou solicitar revisões. Por fim, a geração da planilha para a PROGRAD (Figura \ref{fig:planilha-prograd}) consolida todos os projetos aprovados em formato exportável para envio ao Instituto.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{images/Admin Analise.jpeg}
  \caption{Interface de análise de projetos submetidos pelo administrador.}
  \label{fig:admin-analise}
\end{figure}

\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{images/Planilha Prograd.png}
...
\begin{table}[htb]
  ...
  \begin{tblr}{
    ...
    Testes unitários & 55 testes em 15 arquivos, cobrindo routers tRPC (autenticação, projetos, inscrições, seleção, editais, arquivos, analytics, relatórios) \\
    Taxa de sucesso & 100\% (55/55 testes aprovados sem falhas ou regressões) \\
...
\subsection{Avaliação com Usuários (Estudo Qualitativo)}

Além da validação técnica descrita acima, foi conduzida uma avaliação qualitativa exploratória com dois participantes representando os perfis principais do sistema: um administrador (coordenador de monitoria) e um professor. A escolha por abordagem qualitativa, em detrimento de questionários quantitativos com escala Likert, justifica-se pelo número reduzido de participantes disponíveis (N=2), o que inviabiliza análises estatísticas significativas. Estudos qualitativos com amostras pequenas são apropriados para identificar problemas de usabilidade e obter insights profundos sobre a experiência do usuário~\cite{nielsen1994usability, lazar2017research}.

O foco da avaliação compreendeu o fluxo enfatizado neste trabalho: importação de planejamento \(\rightarrow\) criação/reuso de templates \(\rightarrow\) assinatura de projetos \(\rightarrow\) aprovação administrativa \(\rightarrow\) geração de planilha para o Instituto/PROGRAD.

\subsubsection{Metodologia}

A metodologia adotada combinou três técnicas complementares de coleta de dados:

\begin{enumerate}
  \item \textbf{Walkthrough guiado}: cada participante realizou tarefas predefinidas correspondentes ao seu perfil, seguindo um roteiro estruturado que cobria as funcionalidades principais do sistema. Durante a execução, foram registrados checkpoints de sucesso/falha, tempo gasto e dificuldades observadas.

  \item \textbf{Protocolo Think-Aloud}: os participantes foram instruídos a verbalizar seus pensamentos durante a interação com o sistema, expressando expectativas, dúvidas, surpresas e frustrações. Essa técnica permite capturar a experiência do usuário em tempo real, identificando problemas de usabilidade que não seriam evidentes apenas pela observação.

  \item \textbf{Entrevista semiestruturada}: após o walkthrough, foi conduzida uma entrevista de aproximadamente 15 minutos com cada participante, explorando percepções gerais, pontos positivos e negativos, comparação com o processo manual atual e sugestões de melhoria.
\end{enumerate}

As sessões foram gravadas (tela e áudio) mediante consentimento dos participantes, totalizando aproximadamente 1 hora por participante. A análise dos dados seguiu a técnica de análise temática~\cite{braun2006using}, identificando padrões recorrentes nas verbalizações e respostas.

A avaliação contou com dois participantes representando os perfis principais do sistema: o Prof.\ Rubisley (coordenador de monitoria, perfil administrador) e o Prof.\ Frederico (perfil professor). A Tabela~\ref{tab:participantes} apresenta os participantes e suas atribuições no estudo. O administrador realizou tarefas de importação de planejamento, verificação de notificações automáticas, aprovação de projetos e geração de planilha PROGRAD. O professor executou o fluxo completo de primeiro acesso (recuperação de senha e onboarding com SIAPE, CPF e assinatura digital), visualização do projeto criado pela importação, criação de template reutilizável, edição de projeto do semestre (bolsas, voluntários, atividades), pré-visualização do PDF e assinatura digital com submissão.

\begin{table}[t]
  \centering
  \small
  \caption{Participantes da avaliação qualitativa.}
  \label{tab:participantes}
  \begin{tblr}{
    width=\linewidth,
    colspec={|Q[c,2cm]|Q[c,2cm]|X[l]|},
    hlines,
    vlines
  }
    \textbf{Participante} & \textbf{Perfil} & \textbf{Tarefas Realizadas} \\
    Prof.\ Rubisley & Admin & Importar planejamento, verificar notificações, aprovar projeto, gerar planilha PROGRAD \\
    Prof.\ Frederico & Professor & Primeiro acesso (senha), onboarding, criar template, editar projeto, assinar e submeter \\
  \end{tblr}
\end{table}

O walkthrough foi estruturado em três sessões sequenciais simulando o fluxo real de uso: na primeira sessão, o administrador realizou login, importou a planilha de planejamento do DCC, verificou os professores notificados automaticamente e visualizou os projetos criados com status pendente de assinatura. Na segunda sessão, o professor executou o primeiro acesso via ``Esqueci minha senha'', completou o onboarding, visualizou o projeto criado pela importação, criou/editou template padrão para a disciplina, editou o projeto do semestre e finalizou com pré-visualização do PDF, assinatura digital e submissão. Na terceira sessão, o administrador retornou para visualizar o projeto submetido, analisar os detalhes, aprovar o projeto e gerar a planilha consolidada para o Instituto/PROGRAD.

Após cada walkthrough, foi conduzida uma entrevista semiestruturada de aproximadamente 15 minutos, explorando a experiência geral do participante, comparação com o processo manual atual, estimativa de economia de tempo, barreiras percebidas para adoção em semestre real e sugestões de melhoria. As perguntas foram organizadas por categoria de análise (experiência geral, funcionalidades específicas por perfil, comparação, economia de tempo, adoção e sugestões), conforme apresentado na Tabela~\ref{tab:roteiro-entrevista}. Esta abordagem seguiu as recomendações de Lazar et al.~\cite{lazar2017research} para condução de estudos qualitativos em IHC, permitindo explorar em profundidade as percepções e necessidades dos usuários enquanto mantém foco nas dimensões relevantes para validação do sistema. A estrutura do roteiro foi desenhada para capturar tanto aspectos funcionais quanto percepções subjetivas, possibilitando uma avaliação holística da experiência de uso do sistema por diferentes perfis de usuários acadêmicos.

\begin{table*}[p]
  \centering
  \caption{Roteiro de entrevista semiestruturada por perfil de usuário.}
  \label{tab:roteiro-entrevista}
  \vspace{0.5cm}
  \begin{adjustbox}{max width=\textwidth}
    \begin{tblr}{
      width=\textwidth,
      colspec={|Q[c,2cm]|Q[c,1.8cm]|X[l]|},
      hlines,
      vlines,
      row{1}={font=\bfseries}
    }
      Categoria & Perfil & Pergunta \\
      Experiência geral & Ambos & Descreva sua experiência geral com o sistema. O que mais chamou sua atenção (positivo ou negativo)? \\
      Importação & Admin & A importação da planilha funcionou como esperava? Os erros (se houve) foram bem explicados? \\
      Notificações & Admin & Conseguiu verificar quais professores foram notificados? O feedback do sistema foi suficiente? \\
      Primeiro acesso & Professor & O processo de primeiro acesso (recuperação de senha, onboarding) foi tranquilo? O que poderia melhorar? \\
      Template & Professor & A criação de template foi útil? Usaria em semestres futuros para economizar tempo? \\
      Assinatura digital & Professor & O processo de assinatura digital foi intuitivo? Confia que a assinatura foi registrada corretamente? \\
      PDF gerado & Professor & O PDF gerado atendeu suas expectativas? A pré-visualização foi útil antes de assinar? \\
      Aprovação & Admin & O fluxo de aprovação/rejeição está intuitivo? As informações são suficientes para tomar a decisão? \\
      Planilha PROGRAD & Admin & A planilha gerada atende às necessidades reais? Precisaria de ajustes manuais antes de enviar? \\
      Comparação & Ambos & Comparando com o processo manual atual (planilhas, emails): o que ficou mais fácil? O que ainda parece complicado? \\
      Economia de tempo & Ambos & Quanto tempo você estima que economizaria por semestre usando o sistema? \\
      Adoção & Ambos & Conseguiria usar esse sistema num semestre real? Quais barreiras precisariam ser resolvidas? \\
      Sugestões & Ambos & O que mudaria ou adicionaria ao sistema? Alguma funcionalidade crítica está faltando? \\
    \end{tblr}
  \end{adjustbox}
\end{table*}

\subsubsection{Resultados da Avaliação Qualitativa}

A análise temática das verbalizações e entrevistas, seguindo o método de Braun e Clarke~\cite{braun2006using}, identificou três categorias principais: usabilidade, utilidade e adoção. A Figura~\ref{fig:thematic-map} apresenta o mapa temático resultante da análise, sintetizando os principais achados por categoria.
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
    node distance=1.2cm,
    category/.style={rounded corners, draw=gray!70, fill=blue!10, minimum width=4.5cm, minimum height=1.6cm, align=center, font=\large\bfseries},
    subcategory/.style={rounded corners, draw=gray!50, fill=white, minimum width=4.3cm, text width=4cm, align=left, font=\normalsize},
    title/.style={rounded corners, draw=gray!70, fill=yellow!20, minimum width=12cm, minimum height=1.2cm, align=center, font=\Large\bfseries},
    >={Stealth[length=3mm]}
  ]
    % Título
    \node[title] (title) {Avaliação Qualitativa (N=2, ~1h)};

    % Categorias
    \node[category, below left=1.8cm and 3.5cm of title] (usab) {Usabilidade\\{\normalsize (+4, -1)}};
    \node[category, below=1.8cm of title] (util) {Utilidade\\{\normalsize (+3)}};
    \node[category, below right=1.8cm and 3.5cm of title] (adoc) {Adoção\\{\normalsize (+2, S:3)}};

    % Subcategorias Usabilidade
    \node[subcategory, below=0.7cm of usab] (usab1) {
      \textbullet~Sidebar organizado\\[0.2em]
      \textbullet~Onboarding claro\\[0.2em]
      \textbullet~PDF preview útil\\[0.2em]
      \textbullet~Assinatura intuitiva\\[0.2em]
      {\color{orange}\textbullet~Reset senha$^*$}
    };

    % Subcategorias Utilidade
    \node[subcategory, below=0.7cm of util] (util1) {
      \textbullet~Templates reutilizáveis\\[0.2em]
      \textbullet~Importação automática\\[0.2em]
      \textbullet~Planilha PROGRAD OK
    };

    % Subcategorias Adoção
    \node[subcategory, below=0.7cm of adoc] (adoc1) {
      \textbullet~Projeto continuará\\[0.2em]
      \textbullet~Uso em semestre real\\[0.2em]
      {\color{blue}\textbullet~Notificar professor$^\dagger$}\\[0.2em]
      {\color{blue}\textbullet~Repos. planilha$^\dagger$}
    };

    % Conectores
    \draw[->, line width=1pt] (title.south) -- ++(0,-0.5) -| (usab.north);
    \draw[->, line width=1pt] (title.south) -- (util.north);
    \draw[->, line width=1pt] (title.south) -- ++(0,-0.5) -| (adoc.north);

    % Legenda - positioned below the subcategories
    \node[below=0.8cm of util1, font=\normalsize, text width=12cm, align=center] {
      (+) pontos positivos \quad (-) dificuldades \quad (S) sugestões\\[0.4em]
      $^*$Resolvido durante sessão \quad $^\dagger$Sugestão de melhoria
    };
  \end{tikzpicture}%
  }
  \vspace*{0.3cm}
  \caption{Mapa temático da avaliação qualitativa com usuários.}
  \label{fig:thematic-map}
\end{figure}

\textbf{Perfil Administrador:}
\begin{itemize}
  \item Pontos positivos identificados: O participante destacou a organização lógica do menu lateral, que segue a ordem natural do fluxo de trabalho (projetos, editais, inscrições, seleção, relatórios). A importação de planejamento foi considerada eficiente, com feedback visual claro sobre o número de projetos criados e professores notificados. A planilha PROGRAD gerada foi avaliada como adequada ao formato institucional esperado.

  \item Dificuldades encontradas: Não foram identificadas dificuldades críticas. O participante executou todas as tarefas do roteiro sem interrupções ou erros significativos.

  \item Sugestões de melhoria: Foi sugerida a inclusão de um botão para notificar o professor quando o projeto necessita de ajustes, permitindo comunicação direta via e-mail com campo para mensagem personalizada. Também foi proposta a relocação da funcionalidade de geração de planilha PROGRAD para dentro da seção de Projetos, mantendo cada planilha na etapa correspondente do fluxo.
\end{itemize}

\textbf{Perfil Professor:}
\begin{itemize}
  \item Pontos positivos identificados: A funcionalidade de templates reutilizáveis foi avaliada de forma muito positiva, sendo descrita pelo participante como excelente para economia de tempo em semestres futuros. O processo de onboarding (primeiro acesso) foi considerado claro e o fluxo de assinatura digital intuitivo, com a pré-visualização do PDF antes da assinatura sendo destacada como útil.

  \item Dificuldades encontradas: Houve uma confusão inicial no processo de redefinição de senha, que foi resolvida durante a própria sessão. O participante preencheu incorretamente um campo do formulário (público-alvo), demonstrando um cenário realista de uso que justifica a necessidade da funcionalidade de notificação sugerida pelo administrador.

  \item Sugestões de melhoria: Foi sugerido que o projeto permaneça editável após o envio ao administrador, sendo bloqueado apenas após a aprovação final, facilitando correções quando solicitadas.
\end{itemize}

\textbf{Análise Cruzada:}
\begin{itemize}
  \item Padrões comuns entre os perfis: Ambos os participantes validaram o fluxo principal do sistema, desde a importação até a geração da planilha PROGRAD. A avaliação geral foi positiva, com reconhecimento explícito da contribuição do sistema para a modernização do processo de monitoria. Os participantes concordaram que o sistema está pronto para uso em um semestre real e que continuará evoluindo com contribuições de outros desenvolvedores.

  \item Divergências de percepção: A principal divergência refere-se à localização da funcionalidade de planilha PROGRAD: enquanto o sistema atual a posiciona na seção de Relatórios, o administrador manifestou preferência por mantê-la dentro da seção de Projetos, argumentando que cada etapa deve conter suas próprias planilhas de consolidação.
\end{itemize}

\textbf{Validação dos Objetivos Específicos:}

A Tabela~\ref{tab:validacao-objetivos} apresenta a validação dos objetivos específicos com base nas evidências coletadas durante as sessões de avaliação.

\begin{table*}[t]
  \centering
  \caption{Validação dos objetivos específicos com base na avaliação qualitativa.}
  \label{tab:validacao-objetivos}
  \begin{tblr}{
    width=\textwidth,
    colspec={|X[l,1.2]|Q[c,2.2cm]|X[l,1.5]|},
    hlines,
    vlines,
    row{1}={font=\bfseries}
  }
    Objetivo Específico & Status & Evidência \\
    Digitalizar ciclo completo de projetos & \ding{51} Validado & Fluxo completo executado com sucesso: importação de planejamento, criação de template, assinatura digital, aprovação administrativa e geração de planilha PROGRAD \\
    Automatizar processo seletivo & \ding{51} Parcial & Automação de notificações por e-mail e geração de PDFs confirmada; sugerida melhoria para notificação de ajustes em projetos \\
    Sistematizar alocação de bolsas & \ding{51} Validado & Validação de limites funcionando corretamente; planilha PROGRAD gerada no formato institucional esperado \\
    Eliminar trabalhos manuais repetitivos & \ding{51} Validado & Templates reutilizáveis avaliados como excelentes para economia de tempo em semestres futuros; alto impacto na redução de tarefas repetitivas \\
    Fornecer base analítica para decisões & \ding{51} Parcial & Dashboard funcional com métricas operacionais; analytics avançado identificado como trabalho futuro \\
  \end{tblr}
\end{table*}

\textbf{Intenção de Adoção:}

Ambos os participantes confirmaram a viabilidade do sistema para uso em um semestre real. O administrador destacou que o projeto é contínuo e evoluirá com contribuições futuras de outros desenvolvedores. As barreiras identificadas para adoção plena incluem a necessidade de implementar a funcionalidade de notificação ao professor para solicitação de ajustes e pequenos ajustes de nomenclatura e posicionamento de funcionalidades na interface.
...
\begin{itemize}
  \item \textbf{Amostra reduzida}: a avaliação qualitativa foi conduzida com apenas dois participantes (N=2), o que limita a generalização dos resultados. Embora estudos qualitativos com amostras pequenas sejam válidos para identificar problemas de usabilidade~\cite{nielsen1994usability}, uma avaliação mais abrangente com maior número de usuários seria desejável para confirmar os padrões identificados.

  \item \textbf{Ausência do perfil estudante}: a avaliação focou nos perfis de administrador e professor, não incluindo estudantes. O fluxo de inscrição, seleção e aceite de vagas pelos alunos não foi avaliado empiricamente com usuários reais.

  \item \textbf{Ambiente controlado}: os experimentos foram realizados em ambiente de desenvolvimento/staging, com dados de exemplo. A extrapolação para contextos de maior escala (por exemplo, uso institucional ampliado para toda a UFBA) deve ser feita com cautela.

  \item \textbf{Ausência de comparação quantitativa}: não foram coletadas métricas comparativas diretas (tempo gasto, número de erros) entre o processo manual e o processo automatizado. As estimativas de economia de tempo baseiam-se nas percepções dos participantes.

  \item \textbf{Viés de seleção}: os participantes são professores do Instituto de Computação, com familiaridade com sistemas digitais acima da média. Usuários com menor experiência tecnológica podem ter experiências diferentes.

  \item \textbf{Evolução contínua do sistema}: o Sistema de Monitoria-IC foi concebido como software em evolução. Alterações futuras podem alterar o comportamento observado, exigindo reavaliação periódica.
\end{itemize}

Em síntese, a avaliação experimental combinou validação técnica (55 testes automatizados, 100\% aprovação) com avaliação qualitativa exploratória, fornecendo evidências preliminares de usabilidade e utilidade. A validação em ambiente real de produção, com maior número de usuários e métricas quantitativas de impacto, permanece como trabalho futuro prioritário.
...
Do ponto de vista técnico, o requisito de maior complexidade foi a implementação da assinatura digital com sobreposição no PDF e armazenamento no MinIO, envolvendo manipulação de coordenadas precisas no documento e integração assíncrona com o serviço de arquivos. No âmbito organizacional, enfrentaram-se desafios relacionados à infraestrutura da UFBA (disponibilidade de servidores, configuração de rede, permissões de acesso) e ao alinhamento contínuo de requisitos com os stakeholders, especialmente na definição de formatos de planilhas e fluxos de aprovação compatíveis com procedimentos institucionais existentes.
...
O plano de evolução contempla três horizontes temporais. Num primeiro momento, previsto para os próximos seis meses, propõe-se a realização de um piloto institucional no próprio Instituto de Computação, utilizando o Sistema de Monitoria-IC em paralelo ao processo manual em ao menos um semestre letivo completo. Nessa etapa, serão coletadas métricas quantitativas (tempo de execução de tarefas, número de erros administrativos, volume de retrabalho) e dados qualitativos de satisfação por meio de questionários e entrevistas com professores, estudantes e gestores, a fim de complementar a avaliação técnica aqui apresentada com evidências empíricas de usabilidade e impacto organizacional. O código base de todas as funcionalidades está completo, incluindo o módulo de certificados digitais. Além do piloto, prioriza-se a conclusão da integração com o sistema acadêmico (SIAC) e o desenvolvimento do aplicativo móvel (React Native).
