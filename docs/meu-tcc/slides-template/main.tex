...
\begin{frame}{Fluxo de Processos -- Visão Geral}
    \begin{center}
        \begin{tikzpicture}[
            ...
            every node/.style={rounded corners, draw, align=center, minimum width=5.2cm, minimum height=0.5cm, font=\scriptsize},
            >={Stealth[length=2mm]}
        ]
            \node[fill=gray!20] (fase1) {\textbf{Fase 1:} Planejamento \& Criação de Projetos};
            \node[fill=yellow!20, below=of fase1] (fase2) {\textbf{Fase 2:} Aprovação e Envio para Instituto/PROGRAD};
            \node[fill=orange!20, below=of fase2] (fase3) {\textbf{Fase 3:} Alocação de Bolsas e Edital Interno};
            \node[fill=blue!20, below=of fase3] (fase4) {\textbf{Fase 4:} Inscrições e Seleção de Monitores};
            \node[fill=green!20, below=of fase4] (fase5) {\textbf{Fase 5:} Consolidação Final e Relatório PROGRAD};
            \node[fill=red!15, below=of fase5, dashed] (fase6) {\textbf{Fase 6:} Relatórios e Certificados {\footnotesize(trabalho futuro)}};

            \draw[->] (fase1) -- (fase2);
            \draw[->] (fase2) -- (fase3);
            \draw[->] (fase3) -- (fase4);
            \draw[->] (fase4) -- (fase5);
            \draw[->, dashed] (fase5) -- (fase6);
        \end{tikzpicture}
    \end{center}
    \vspace{0.1cm}
    \small
...
\begin{frame}{Resultados da Avaliação}
    \begin{center}
        \textbf{Avaliação com Professores (Questionário Likert 1-5)}
        \vspace{0.3cm}

        % TODO: Substituir esta tabela pelos resultados reais
        \begin{tabularx}{0.95\linewidth}{|l|X|c|}
            \hline
            \textbf{Q} & \textbf{Aspecto Avaliado} & \textbf{Média} \\
            \hline
            Q1-Q4 & Expectativa (importação, template, PDF, assinatura) & \textit{TODO} \\
            \hline
            Q5-Q7 & Avaliação (assinatura, feedback, estados) & \textit{TODO} \\
            \hline
            Q8-Q9 & Impacto (planilha PROGRAD, transparência) & \textit{TODO} \\
            \hline
            Q10 & Satisfação geral com Fases 1-2 & \textit{TODO} \\
            \hline
            Q11-Q12 & Intenção de uso e recomendação (Sim/Não) & \textit{TODO} \\
            \hline
        \end{tabularx}

        \vspace{0.5cm}
        \small
        \textbf{Validação Técnica:} 57 testes automatizados | 100\% aprovação | 1.07s execução
    \end{center}
\end{frame}
...
\begin{frame}{Contribuições do Trabalho}
    \begin{enumerate}
        ...
        \item \textbf{Estudo de caso:} Transformação digital em instituição pública com evidências técnicas (57 testes, 100\% aprovação)
    ...
    \begin{columns}[T]
        \begin{column}{0.46\textwidth}
            ...
            \begin{itemize}
                ...
                \item Módulo de certificados pendente (Fase 6)
        ...
        \begin{column}{0.46\textwidth}
            ...
            \begin{itemize}
                ...
                \item Módulo de certificados digitais
