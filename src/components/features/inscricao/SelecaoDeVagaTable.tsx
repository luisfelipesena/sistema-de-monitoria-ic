...
const SelecaoDeVagaTable: React.FC<SelecaoDeVagaTableProps> = ({
  vagas,
  onSelecionar,
}) => {
  return (
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {vagas.map((vaga) => (
              <tr key={vaga.id}>
                ...
                <td className="px-6 py-4">
                  {vaga.selecionado ? (
                    <span className="inline-flex items-center gap-1 px-4 py-1 rounded-full bg-[#D1D5DB] text-white text-sm cursor-default">
                      {' '}
                      Selecionado
                    </span>
                  ) : (
