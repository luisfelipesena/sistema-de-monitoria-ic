...
export const FileUploader = forwardRef<HTMLInputElement, FileUploaderProps>(
  ({ onFileSelect, allowedTypes = [], maxSizeInMB = 5, className, selectedFile }, ref) => {
    const [error, setError] = useState<string | null>(null)
    const [internalSelectedFile, setInternalSelectedFile] = useState<File | null>(selectedFile || null)
                ...
                {allowedTypes.length > 0
                  ? `Tipos permitidos: ${allowedTypes
                      .map((type) => {
                        ...
                        return extensions[type as keyof typeof extensions] || type
                      })
                      .join(", ")}`
                  : ""}
