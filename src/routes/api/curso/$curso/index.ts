...
export const APIRoute = createAPIFileRoute('/api/curso/$curso')({
  GET: createAPIHandler(async ({ params }) => {
    try {
      const curso = await db.query.cursoTable.findFirst({
        where: eq(cursoTable.id, Number(params.curso))
      });
    ...
    withRoleMiddleware(['admin'], async (ctx) => {
      try {
        ...
        const result = await db.update(cursoTable)
          ...
          .where(eq(cursoTable.id, Number(ctx.params.curso)))
          .returning();
  ...
  DELETE: createAPIHandler(
    withRoleMiddleware(['admin'], async (ctx) => {
      try {
        const result = await db.delete(cursoTable)
          .where(eq(cursoTable.id, Number(ctx.params.curso)))
          .returning();
