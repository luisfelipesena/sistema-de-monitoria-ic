...
export const APIRoute = createAPIFileRoute('/api/files/admin/delete')({
  POST: async ({ request }) => {
    try {
      ...
      return json({ message: 'Arquivo excluído com sucesso' }, { status: 200, headers: { 'Content-Type': 'application/json' } });
      ...
      return json({ error: 'Erro interno do servidor ao excluir o arquivo' }, { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
  },
}); 