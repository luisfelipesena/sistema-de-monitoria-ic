import { GENERO_FEMININO, GENERO_MASCULINO, GENERO_OUTRO, type AnexoIVInputs } from '@/types'
...
function formatEnderecoCompleto(e: AnexoIVInputs['monitor']['endereco']): string {
  const parts = [e.rua, e.numero ? String(e.numero) : '', e.bairro, e.cidade, e.estado, e.cep].filter(Boolean)
  return parts.join(', ')
}

export function AnexoIVInscricaoVoluntarioTemplate({ data }: { data: AnexoIVInputs }) {
  ...
  return (
    <Document>
      <Page size="A4" style={anexoStyles.page}>
        <UfbaPrograd />
        <Text style={anexoStyles.formTitle}>ANEXO IV – FORMULÁRIO DE INSCRIÇÃO DE MONITOR VOLUNTÁRIO</Text>
        ...
        {/* ==================== SECTION 3: DECLARAÇÃO DO MONITOR ==================== */}
        <View style={[anexoStyles.table, { marginTop: 6 }]}>
          <View style={anexoStyles.sectionHeaderRow}>
            <Text style={anexoStyles.sectionHeaderText}>3. DECLARAÇÃO DO(A) MONITOR(A)</Text>
          </View>

          <View style={anexoStyles.row}>
            <View style={anexoStyles.cellFull}>
              <Text>
                3.1 Declara ter cursado com aprovação o componente curricular do qual será monitor(a): Sim (
          ...
          <View style={anexoStyles.row}>
            <View style={anexoStyles.cellFull}>
              <Text>
                3.2 Em caso negativo no item anterior, declara ter cursado com aprovação componente curricular
          ...
          <View style={anexoStyles.row}>
            <View style={anexoStyles.cellFull}>
              <Text>
                3.2.1 Informar o código e o nome do componente curricular equivalente:{' '}
        ...
        <Text style={[anexoStyles.formTitle, { marginTop: 16 }]}>TERMO DE COMPROMISSO DO MONITOR VOLUNTÁRIO</Text>
        ...
        <Text style={anexoStyles.paragraph}>
          ...
          declaro ter ciência das obrigações inerentes à qualidade de Monitor(a) Voluntário(a) do Programa de Monitoria
          da Universidade Federal Bahia e comprometo-me a:
        </Text>
        ...
        <Text style={anexoStyles.clausulaListItem}>
          ...
          definidas no EDITAL PROGRAD/UFBA Nº 01/{projeto.ano};
        </Text>
        ...
        <Text style={anexoStyles.clausulaListItem}>
          4. Ter cursado com aprovação o componente curricular ou equivalente ao qual se vincula a atividade de monitoria;
        </Text>
